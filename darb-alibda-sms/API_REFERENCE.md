...
> **التوثيق التفاعلي (Swagger):** يوجد الآن توثيق كامل وقابل للتجربة على `/docs`،
> ومواصفة OpenAPI على `/openapi.yaml`. هذا الملف يغطي مسارات المعلم فقط،
> بينما تغطي مواصفة OpenAPI جميع المسارات (المعلم وولي الأمر).
...
- `success`: قيمة منطقية (`true` عند النجاح، `false` عند الخطأ)
- `message`: رسالة نصية قصيرة
- `data`: محتوى الاستجابة عند النجاح
- `errors`: تفاصيل الخطأ عند الفشل
