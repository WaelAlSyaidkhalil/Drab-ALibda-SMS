<?php

return [
    'app_name' => 'Dar Al_Ibdaa',
    'navigation' => [
        'school_management' => 'School Management',
        'student_management' => 'Student Management',
        'teacher_management' => 'Teacher Management',
        'scheduling' => 'Scheduling',
        'communication' => 'Communication',
        'feedback_center' => 'Feedback Center',
        'user_management' => 'User Management',
        'system' => 'System',
    ],

    'pages' => [
        'school_info' => 'School Info',
        'attendance_calendar' => 'Attendance Calendar',
        'teacher_timetable' => 'Teacher Timetable',
        'timetable' => 'Timetable',
        'time_slots' => 'Time Slots',
        'complaints' => 'Complaints',
        'suggestions' => 'Suggestions',
        'students_info' => 'Students Info',
        'student_enrollments' => 'Student Enrollments',
        'school_classes' => 'School Classes',
        'subjects' => 'Subjects',
        'terms' => 'Terms',
        'term' => 'Term',
        'teachers' => 'Teachers',
        'news' => 'News',
        'roles' => 'Roles',
        'schedules' => 'Schedules',
        'users' => 'Users',
        'sections' => 'Sections',
        'section' => 'Section',
        'subject_components' => 'Subject Components',
        'subject_component' => 'Subject Component',
        'subject_result' => 'Subject Result',
        'subject_component_mark' => 'Subject Compoinent Mark',
        'attendance' => 'Attendance',
        'teacher_attendance' => 'Teacher Attendance',
        'teacher_attendance_calendar' => 'Teacher Attendance Calendar',
        'schedule' => 'Schedule',
        'time_slot' => 'Time Slot'
    ],

    'widgets' => [
        'school_overview' => 'School Overview',
        'school_overview_description' => 'Key numbers for the current academic year',
        'students' => 'Students',
        'teachers' => 'Teachers',
        'sections' => 'Sections',
        'attendance' => 'Attendance',
        'success_rate' => 'Success Rate',
        'daily_statistics' => 'Daily Statistics',
        'daily_statistics_description' => 'Snapshot of the current day',
        'students_absent' => 'Students Absent',
        'teachers_absent' => 'Teachers Absent',
        'new_suggestions' => 'New Suggestions',
        'new_complaints' => 'New Complaints',
        'school_rates' => 'School Rates',
        'school_rates_description' => 'Overall success rate for the current academic year',
        'failure_rate' => 'Failure Rate',
        'success_rate' => 'Success Rate',
        'subject_success_chart' => 'Subject Success Chart',
        'subject_success_chart_description' => 'Average performance by subject',
        'average' => 'Average %',
        'top_students' => 'Top Students',
        'most_absent_sections' => 'Most Absent Sections',
        'student_status' => 'Student Status',
        'student_status_description' => 'Number of students by status',
        'graduated' => 'Graduated Students',
        'section_success_rate' => 'Section Success Rate',
    ],

    'labels' => [
        'account' => 'Account',
        'teacher' => 'Teacher',
        'term' => 'Term',
        'section' => 'Section',
        'subject' => 'Subject',
        'class' => 'Class',
        'start_time' => 'Start Time',
        'end_time' => 'End Time',
        'period' => 'Period',
        'time_range' => 'Time Range',
        'duration' => 'Duration',
        'user' => 'User',
        'status' => 'Status',
        'active' => 'Active',
        'inactive' => 'Inactive',
        'role' => 'Role',
        'acknowledged' => 'Acknowledged',
        'feedback' => 'Feedback',
        'not_acknowledged' => 'Not Acknowledged',
        'save_schedule' => 'Save Schedule',
        'schedule_editor' => 'Schedule Editor',
        'update_complaint' => 'Update Complaint',
        'update_suggestion' => 'Update Suggestion',
        'name' => 'Name',
        'phone' => 'Phone',
        'email' => 'Email',
        'address' => 'Address',
        'website' => 'Website',
        'save_success' => 'Data saved successfully',
        'missing_selection' => 'Missing Selection',
        'missing_selection_message_for_student_timetable' => 'Please select Term and Section first.',
        'missing_selection_message_for_teacher_timetable' => 'Please select Teacher and Class first.',
        'not_available' => 'N/A',
        'personal_information' => 'Personal Information',
        'account_details' => 'Account Details',
        'employment_information' => 'Employment Information',
        'official_information' => 'Official Information',
        'contact_information' => 'Contact Information',
        'father_name' => 'Father Name',
        'mother_name' => 'Mother Name',
        'birth_date' => 'Birth Date',
        'gender' => 'Gender',
        'employee_number' => 'Employee Number',
        'hire_date' => 'Hire Date',
        'employment_type' => 'Employment Type',
        'grade' => 'Grade',
        'specialization' => 'Specialization',
        'experience_years' => 'Experience Years',
        'registry_number' => 'Registry Number',
        'national_id' => 'National ID',
        'alternative_phone' => 'Alternative Phone',
        'student_details' => 'Student Details',
        'parent_account_details' => 'Parent account Details',
        'official_details' => 'Official Details',
        'parent_name' => 'Parent name',
        'student' => 'Student',
        'academic_year' => 'Academic Year',
        'enrollment_date' => 'Enrollment Date',
        'final_result' => 'Final Result',
        'final_average' => 'Final Average',
        'notes' => 'Notes',
        'mark' => 'Mark',
        'percentage' => 'Percentage',
        'component' => 'Component',
        'yearly_mark' => 'Yearly Mark',
        'subject_component_marks' => 'Subject Component Marks',
        'subject_results' => 'Subject Results',
        'readers' => 'Readers',
        'audience' => 'Audience',
        'all_audiences' => 'All audiences',
        'unknown' => 'Unknown',
        'pass' => 'Pass',
        'fail' => 'Fail',
        'class_section' => 'Class & Section',
        'subjects_count' => 'Subjects count',
        'years_0_5' => '0-5 years',
        'years_6_10' => '6-10 years',
        'years_11_20' => '11-20 years',
        'years_21_plus' => '21+ years',
        'subject_details' => 'Subject Details',
        'news_information' => 'News Information',
        'images' => 'Images',
        'videos' => 'Videos',
        'all' => 'All',
        'parents' => 'Parents',
        'students' => 'Students',
        'teachers' => 'Teachers',
        'optional' => 'Optional',
        'class_type' => 'Class',
        'capacity' => 'Capacity',
        'student_count' => 'Student Count',
        'days_remaining' => 'Days Remaining',
        'code' => 'Code',
        'full_mark' => 'Full Mark',
        'pass_mark' => 'Pass Mark',
        'description' => 'Description',
        'component_type' => 'Component Type',
        'out_of' => 'Out of',
        'order' => 'Order',
        'school_name' => 'School Name',
        'term1_mark' => 'Term 1 Mark',
        'term2_mark' => 'Term 2 Mark',
        'result' => 'Result',
        'full_name' => 'Full Name',
        'first_name' => 'First Name',
        'last_name' => 'Last Name',
        'age' => 'Age',
        'password' => 'Password',
        'reset_password' => 'Reset password',
        'reset_password_confirm' => 'A new password will be generated and shown once. The current password will stop working.',
        'reset_password_done' => 'New password generated',
        'reset_password_copy' => 'Copy it now — it cannot be shown again.',
        'day' => 'Day',
        'time_slot' => 'Time Slot',
        'minutes' => 'min',
        'readers_count' => 'Readers Count',
        'title' => 'Title',
        'body' => 'Body',
        'created_at' => 'Created At',
        'response' => 'Response',
        'resolved_at' => 'Resolved At',
        'date' => 'Date',
        'reason' => 'Reason',
        'rank' => 'Rank',
        'average' => 'Average',
        'absence_percentage' => 'Absence %',
        'promoted' => 'Promoted',
        'repeated' => 'Repeated',
        'withdrawn' => 'Withdrawn',
        'graduated' => 'Graduated',
        'transferred' => 'Transferred',
        'success_rate' => 'Success Rate',
        'terms' => 'Terms',
        'subjects' => 'Subjects',
        'student_enrollments' => 'Student Enrollments',
        'time_slots' => 'Time Slots',
        'schedules' => 'Schedules',
        'news' => 'News',
        'suggestions' => 'Suggestions',
        'complaints' => 'Complaints',
        'roles' => 'Roles',
        'users' => 'Users',
        'top_students' => 'Top Students',
        'section_success_rate' => 'Section Success Rate',
        'subject_components' => 'Subject Components',
        'timetable_generated' => 'Timetable Generated successfully',
        'timetable_generation_failed' => 'Timetable Generation failed',
        'timetable_published_success' => 'Timetable published successfully',
        'timetable_published_success_teacher' => 'Teacher timetable has been published to all teachers.',
        'timetable_published_success_student' => 'Student timetable has been published to all students.',
        'timetable_published_title' => 'Timetable Published',
        'timetable_published_body_student' => 'The student timetable has been published. Please check your schedule.',
        'timetable_published_body_teacher' => 'The teacher timetable has been published. Please review your assigned schedule.',
        'timetable_publish_failed_title' => 'Publish failed',
        'timetable_publish_failed_body' => 'No recipients were found to send the timetable notification.',
        'password_confirmation' => 'Password Confirmation',
    ],

    'buttons' => [
        'save' => 'Save',
        'update' => 'Update',
        'delete' => 'Delete',
        'acknowledge' => 'Acknowledge',
        'publish_timetable' => 'Publish Timetable',
        'create_section' => 'Create Section',
        'generate_timetable' => 'Generate Timetable',
        'create_term' => 'Create Term',
        'create_subject' => 'Create Subject',
        'create_subject_component' => 'Create Subject Component',
        'edit_subject_component' => 'Edit Subject Component',
        'create_student_enrollment' => 'Create Student Enrollment',
        'edit_student_enrollment' => 'Edit Student Enrollment',
        'new_subject_result' => 'New Subject Result',
        'publish_timetable' => 'Publish Timetable',
        'new_subject_component_mark' => 'New Subject Component Mark',
        'create_student' => 'Create Student',
        'create_teacher' => 'Create Teacher',
        'create_schedule' => 'Create Schedule',
        'create_time_slot' => 'Create Time Slot',
        'create_news' => 'Create News',
        'create_user' => 'Create User',
        'edit' => 'Edit',
        'delete' => 'Delete'
    ],

    'enums' => [
        'attendance_status' => [
            'present' => 'Present',
            'absent' => 'Absent',
            'late' => 'Late',
        ],

        'audience_types' => [
            'all' => 'All',
            'teachers' => 'Teachers',
            'parents' => 'Parents',
            'students' => 'Students',
        ],

        'complaint_status' => [
            'pending' => 'Pending',
            'in_progress' => 'In Progress',
            'resolved' => 'Resolved',
        ],

        'day_of_week' => [
            'day' => 'Day',
            'sunday' => 'Sun',
            'monday' => 'Mon',
            'tuesday' => 'Tue',
            'wednesday' => 'Wed',
            'thursday' => 'Thu',
        ],

        'gender' => [
            'male' => 'Male',
            'female' => 'Female',
        ],

        'mark_result' => [
            'pass' => 'Pass',
            'fail' => 'Fail',
            'pending' => 'Pending',
        ],

        'student_status' => [
            'active' => 'Active',
            'promoted' => 'Promoted',
            'repeated' => 'Repeated',
            'transferred' => 'Transferred',
            'graduated' => 'Graduated',
            'withdrawn' => 'Withdrawn',
        ],

        'subject_component_type' => [
            'written' => 'Written',
            'oral' => 'Oral',
            'practical' => 'Practical',
        ],

        'suggestion_status' => [
            'pending' => 'Pending',
            'reviewed' => 'Reviewed',
            'accepted' => 'Accepted',
            'rejected' => 'Rejected',
        ],

        'term_status' => [
            'active' => 'Active',
            'upcoming' => 'Upcoming',
            'completed' => 'Completed',
        ],

        'term_type' => [
            'first_term' => 'First Term',
            'second_term' => 'Second Term',
        ],

        'time_slot' => [
            'first' => 'First Slot',
            'second' => 'Second Slot',
            'third' => 'Third Slot',
            'fourth' => 'Fourth Slot',
            'fifth' => 'Fifth Slot',
            'sixth' => 'Sixth Slot',
            'seventh' => 'Seventh Slot',
        ],

        'user_role' => [
            'admin' => 'Admin',
            'teacher' => 'Teacher',
            'student' => 'Student',
            'parent' => 'Parent',
        ],
    ],

    'notifications' => [
        'student_enrolled_title' => 'Student enrolled',
        'student_enrolled_body' => ':student has been enrolled in :section for the :year academic year.',
        'student_enrollment_status_updated_title_success' => 'Congratulations!',
        'student_enrollment_status_updated_title_failed' => 'Result update',
        'student_enrollment_status_updated_title_withdrawn' => 'Enrollment withdrawn',
        'student_enrollment_status_updated_title' => 'Enrollment status updated',
        'student_enrollment_status_updated_body_promoted' => ':student, great job! You passed and have been promoted with a final average of :average.',
        'student_enrollment_status_updated_body_graduated' => ':student, congratulations! You have graduated with a final average of :average.',
        'student_enrollment_status_updated_body_repeated' => ':student, you have not passed this year. Your final average is :average.',
        'student_enrollment_status_updated_body_withdrawn' => ':student, your enrollment has been withdrawn. Final average: :average.',
        'student_enrollment_status_updated_body' => ':student enrollment status is now :status.',
        'student_enrollment_status_updated_body_with_average' => ':student enrollment status in :section is now :status with a final average of :average.',
        'student_enrollment_status_updated_body_with_result' => ':student enrollment status in :section is now :status and the final result is :result.',
        'teacher_created_title' => 'Teacher account created',
        'teacher_created_body' => ':teacher has been created and can now access the system.',
        'teacher_absent_title' => 'Teacher absent',
        'teacher_absent_body' => ':teacher was marked absent on :date.',
        'suggestion_status_updated_title' => 'Suggestion status updated',
        'suggestion_status_updated_body' => 'Your suggestion ":title" status is now :status.',
        'complaint_status_updated_title' => 'Complaint status updated',
        'complaint_status_updated_body' => 'Your complaint ":title" status is now :status.',
        'timetable_published_title' => 'Timetable Published',
        'timetable_publish_failed_title' => 'Publish failed',
        'timetable_publish_failed_body_no_schedule' => 'No timetable was found to publish.',
        'timetable_publish_failed_body_no_students' => 'No students were found to send the timetable notification.',
        'timetable_publish_failed_body_no_teacher_user' => 'No user was found for the teacher to send the timetable notification.',
        'timetable_published_success_title' => 'Timetable Published',
        'timetable_published_success_body' => 'The timetable has been published.',
    ],

    'validation' => [
        'day_required' => 'This field is required.',
        'section_id_required' => 'This field is required.',
        'section_id_unique' => 'This field must be unique.',
        'subject_id_required' => 'This field is required.',
        'subject_id_unique' => 'This field must be unique.',
        'teacher_time_conflict' => 'A teacher cannot be assigned to two classes at the same time.',
        'term1_mark_numeric' => 'This field must be a number.',
        'term1_mark_minValue' => 'This field must be at least :min.',
        'term1_mark_maxValue' => 'This field must be at most :max.',
        'term2_mark_numeric' => 'This field must be a number.',
        'term2_mark_minValue' => 'This field must be at least :min.',
        'term2_mark_maxValue' => 'This field must be at most :max.',
        'yearly_mark_numeric' => 'This field must be a number.',
        'yearly_mark_minValue' => 'This field must be at least :min.',
        'yearly_mark_maxValue' => 'This field must be at most :max.',
        'in' => 'The specified value is not valid.',
        'end_time_after_start' => 'End time must be after start time.',
        'overlapping_time_slot' => 'This time slot overlaps with an existing time slot.',
        'academic_year_regex' => 'The academic year must be in the format YYYY-YYYY.',
        'academic_year_unique' => 'The academic year must be unique for this term type.',
        'title_required' => 'Title is required.',
        'title_max' => 'Title may not exceed 255 characters.',
        'audience_required' => 'Audience is required.',
        'body_required' => 'Body is required.',
        'student_id_required' => 'Student is required.',
        'status_required' => 'Status is required.',
        'date_required' => 'Date is required.',
        'reason_max' => 'Reason may not exceed 500 characters.',
        'name_required' => 'Name is required.',
        'name_max' => 'Name may not exceed 255 characters.',
        'capacity_required' => 'Capacity is required.',
        'subject_component_id_required' => 'Subject component is required.',
        'term_id_required' => 'Term is required.',
        'mark_required' => 'Mark is required.',
        'mark_numeric' => 'Mark must be a number.',
        'academic_year_required' => 'Academic year is required.',
        'academic_year_max' => 'Academic year may not exceed 255 characters.',
        'enrollment_date_required' => 'Enrollment date is required.',
        'final_result_required' => 'Final result is required.',
        'final_average_numeric' => 'Final average must be a number.',
        'notes_max' => 'Notes may not exceed 255 characters.',
        'first_name_required' => 'First name is required.',
        'first_name_max' => 'First name may not exceed 255 characters.',
        'last_name_required' => 'Last name is required.',
        'last_name_max' => 'Last name may not exceed 255 characters.',
        'father_name_max' => 'Father name may not exceed 255 characters.',
        'mother_name_max' => 'Mother name may not exceed 255 characters.',
        'gender_required' => 'Gender is required.',
        'national_id_unique' => 'National ID must be unique.',
        'registry_number_required' => 'Registry number is required.',
        'registry_number_unique' => 'Registry number must be unique.',
        'is_active_boolean' => 'The active flag must be true or false.',
        'code_required' => 'Code is required.',
        'code_unique' => 'Code must be unique.',
        'class_id_required' => 'Class is required.',
        'teacher_id_required' => 'Teacher is required.',
        'full_mark_required' => 'Full mark is required.',
        'full_mark_numeric' => 'Full mark must be a number.',
        'full_mark_min' => 'Full mark must be at least :min.',
        'pass_mark_required' => 'Pass mark is required.',
        'pass_mark_numeric' => 'Pass mark must be a number.',
        'pass_mark_min' => 'Pass mark must be at least :min.',
        'pass_mark_lte' => 'Pass mark must be less than or equal to full mark.',
        'feedback_required' => 'Feedback is required.',
        'phone_required' => 'Phone is required.',
        'phone_unique' => 'Phone must be unique.',
        'phone_tel' => 'Phone format is invalid.',
        'email_required' => 'Email is required.',
        'email_email' => 'Email format is invalid.',
        'email_unique' => 'Email must be unique.',
        'role_id_required' => 'Role is required.',
        'password_min' => 'Password must be at least 8 characters.',
        'password_max' => 'Password may not exceed 255 characters.',
        'password_confirmation_same' => 'Password confirmation does not match.',
        'password_confirmation_min' => 'Password confirmation must be at least 8 characters.',
        'password_confirmation_max' => 'Password confirmation may not exceed 255 characters.',
        'type_required' => 'Type is required.',
        'start_date_date' => 'Start date is invalid.',
        'end_date_date' => 'End date is invalid.',
        'start_time_required' => 'Start time is required.',
        'end_time_required' => 'End time is required.',
        'employee_number_unique' => 'Employee number must be unique.',
        'experience_years_numeric' => 'Experience years must be a number.',
        'experience_years_min' => 'Experience years must be at least :min.',
        'national_id_required' => 'National ID is required.',
        'result_in' => 'The specified value is not valid.',
    ],
];
